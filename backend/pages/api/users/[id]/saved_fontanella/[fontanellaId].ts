    ...
    case "DELETE":
      try {
        if (!fontanellaId || typeof fontanellaId !== "string") {
          return res
            .status(400)
            .json({ error: "fontanellaId è richiesto per la cancellazione" });
        }

        const deleted = await SavedFontanella.findOneAndDelete({
          userId: id,
          fontanellaId,
        });
        if (!deleted) {
          return res
            .status(404)
            .json({ error: "Fontanella non trovata per questo utente" });
        }
        ...
        return res
          .status(500)
          .json({ error: "Errore nella rimozione della fontanella" });
      }
